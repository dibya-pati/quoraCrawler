...
def scrapeLandingPage():
    ...
    page_count = 0
    if (DEBUG): print "In Landing Page"
    ...
    scholarProfiles = open('scholarProfile'+str(int(time.time()))+'.csv', mode='w')
...
#die on timeout
    while depth < max_depth :
        ...
        try:
            ...
            profileBlocks = browser.find_elements_by_xpath('//*[@id="gsc_sa_ccl"]/*')
            for profileContentsXpath in profileBlocks:
                if DEBUG:print '+++++++++++++++++++++++++++++++++++++'
                try:
                    user_url = profileContentsXpath.find_element_by_xpath("./div/h3/a").get_attribute('href')
                    user_name =  profileContentsXpath.find_element_by_xpath("./div/h3/a").text
                    affiliation = profileContentsXpath.find_element_by_xpath("./div/div[1]").text
                    verification_id = profileContentsXpath.find_element_by_xpath("./div/div[2]").text
                    citation_text = profileContentsXpath.find_element_by_xpath("./div/div[3]").text
                    tag_text = profileContentsXpath.find_element_by_xpath("./div/div[4]").text
                    if DEBUG:print user_name,user_url,affiliation,verification_id,citation_text,tag_text
            ...
            button = browser.find_element_by_xpath('//*[@id="gsc_authors_bottom_pag"]/div/button[2]')
        except:
            break
